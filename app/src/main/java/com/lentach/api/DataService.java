...
import com.lentach.util.IDCommentsComporator;
import com.lentach.util.IDUsersComporator;
import com.lentach.api.serverApi.ServerApiManager;
...
public class DataService {
    ...
    public static DataService init() {
        if (dataServiceSingleton == null) {
            dataServiceSingleton = new DataService();
        }
        return dataServiceSingleton;
    }

    public void getPostsFromWall(final Context context, final onRequestResult listener,int count,int offset) {

        VKRequest vkRequest = new VKApiWall().get(VKParameters.from(VKApiConst.OWNER_ID,context.getResources().getInteger(R.integer.group_id),VKApiConst.COUNT,count,VKApiConst.OFFSET,offset));
    ...
    public void getVideoFromIdVidach(final Context context, final onRequestVideoFromId  listener, int videoId) {

        VKRequest request1234 = new VKRequest
                ("video.get",
                        VKParameters.from(VKApiConst.OWNER_ID,
                                context.getResources().getInteger(R.integer.group_video_id),
                                "videos",context.getResources().getInteger(R.integer.group_video_id) + "_" + videoId,VKApiConst.EXTENDED,"1"));
        request1234.executeWithListener(new VKRequest.VKRequestListener() {
            @Override
            public void onComplete(VKResponse response) {
                super.onComplete(response);
            ...
            public void onError(VKError error) {
                 super.onError(error);
                Toast.makeText(context,"Для того чтобы смотреть видео внутри приложения вам необходимо авторизоваться!",Toast.LENGTH_SHORT).show();
                listener.onRequestVideoFromId(null);
            }
        });
    }
    ...
    public void getVideoPostsFromWall(final Context context, final onRequestResult listener,int count,int offset) {

        VKRequest vkRequest = new VKApiWall().get(VKParameters.from(VKApiConst.OWNER_ID,context.getResources().getInteger(R.integer.group_video_id),VKApiConst.COUNT,count,VKApiConst.OFFSET,offset));
        vkRequest.executeWithListener(new VKRequest.VKRequestListener() {
            @Override
            public void onComplete(VKResponse response) {
                super.onComplete(response);
