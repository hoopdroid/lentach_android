 ...
 */
public class HotPostsRVAdapter extends RecyclerView.Adapter<HotPostsRVAdapter.ViewHolder> {
    ...
    public HotPostsRVAdapter(Context context, List<Post> PostsList) {
    ...
    private void removeUnusedPosts() {
        for (int i = 0; i < postsList.size() ; i++) {
            if(postsList.get(i).getIsPinned()!=null) {
                postsList.remove(i);
                continue;
            }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_hot_post, viewGroup, false);
    ...
    private void loadPhoto(ViewHolder viewHolder, int i) {
        if(postsList.get(i).getAttachments().get(0).getPhoto()!=null){
            Picasso.with(context).load(postsList.get(i).getAttachments().get(0).getPhoto().getPhoto604()).error(R.drawable.lentach_placeholder)
